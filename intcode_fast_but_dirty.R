intfast=function(X,inputs=c(),n=1,m=1,rbase=0,verbose=FALSE,aggregated=FALSE){
  options(digits=22)
  x = rep(0,1e6)
  x[seq_along(X)]=X
  out = 0
  while(x[n]%%100!=99){
    ...
    if(op==1){
      ...
      if(m>length(inputs)){
        return(list(op=x,out=`if`(aggregated,paste(out[-1],collapse=""),out[-1]),n=n,m=m,rb=rbase,status=1))
      }else{
  ...
  list(op=x,out=`if`(aggregated,paste(out[-1],collapse=""),out[-1]),n=n,m=m,rb=rbase,status=0)
}